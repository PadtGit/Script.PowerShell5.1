...
function Resolve-StandaloneAdobePackagePath {
    [CmdletBinding()]
    [OutputType([string])]
    param(
        [AllowEmptyString()]
    ...
    if (-not [string]::IsNullOrWhiteSpace($PackagePath)) {
        ...
        return [System.IO.Path]::GetFullPath((Join-Path -Path $ScriptDirectory -ChildPath $PackagePath))
    }

    foreach ($CandidateName in @('AcrobatInstaller.msi', 'AcrobatInstaller.exe')) {
        $CandidatePath = Join-Path -Path $ScriptDirectory -ChildPath $CandidateName
    ...
    $DiscoveredPackages = @(
        Get-ChildItem -LiteralPath $ScriptDirectory -File -ErrorAction SilentlyContinue |
            Where-Object {
                $_.Extension -in @('.msi', '.exe') -and
                $_.BaseName -match 'Adobe|Acrobat|Reader'
            }
    )

    if ($DiscoveredPackages.Count -eq 1) {
        return $DiscoveredPackages[0].FullName
    }

    if ($DiscoveredPackages.Count -gt 1) {
        throw (
            'Multiple Adobe installer packages were found beside the script. Re-run with -PackagePath. Candidates: {0}' -f
            ($DiscoveredPackages.FullName -join '; ')
        )
    }

    return (Join-Path -Path $ScriptDirectory -ChildPath 'AcrobatInstaller.msi')
}
...
try {
    $ResolvedPackagePath = Resolve-StandaloneAdobePackagePath `
        -PackagePath $PackagePath `
